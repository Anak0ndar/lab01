...
Matrix *createMatrixIn(size_t element_size, const struct functions *f) {
    ...
    m->length = length;
    m->height = height;
    m->matrix = (void*)calloc(length * height, element_size);
    return m;
}
...
Matrix *TranspMatrix(Matrix* mat1) {
    Matrix *trans = createMatrix(mat1->height, mat1->length, mat1->element_size, mat1->functions);
    void *m1, *res;
    for (int i = 0; i < mat1->length; ++i) {
        for (int j = 0; j < mat1->height; ++j) {
            m1 = (void*)(mat1->matrix + ((i * mat1->length) + j) * mat1->element_size);
            res = (void*)(trans->matrix + ((j * trans->length) + i) * trans->element_size);
            mat1->functions->assign(m1, res);
        }
    }
    return trans;
}
