...
void floatTest() {
    ...
    Matrix* m1 = createMatrix(2, 2, sizeof(float), functionsFloat);
    Matrix* m2 = createMatrix(2, 2, sizeof(float), functionsFloat);
    ...
    for (int i = 0; i < m1->length; ++i) {
        for (int j = 0; j < m2->height; ++j) {
            for (int k = 0; k < m1->height; ++k) {
                ...
                *(float*)res = *(float*)res + *(float*)mat1 * *(float*)mat2;
            }
        }
    }
    ...
    for (int i = 0; i < m1->length; ++i) {
        for (int j = 0; j < m1->height; ++j) {
            ...
            *(float*)res = *(float*)mat1 + *(float*)mat2;
        }
    }
    printf("\nFirst summed matrix\n");
    printMatrix(sum1);
...
void complexTest() {
    ...
    functionsComplex->assign = complexAssign;
    ...
    if (mult1->length != mult2->length || mult1->height != mult2->height) {
        ...
        for (int i = 0; i < mult1->length; ++i) {
            for (int j = 0; j < mult1->height; ++j) {
                ...
                complex* n1 = t1;
                complex* n2 = t2;
                if (n1->realPart != n2->realPart || n1->complexPart != n2->complexPart)
                    printf("Error, elements %d,%d in multiplied matrices aren't equal\n", i, j);
            }
        }
    }
    ...
    for (int i = 0; i < m1->length; ++i) {
        for (int j = 0; j < m1->height; ++j) {
            ...
            res = (void*)(scalar1->matrix + ((i * scalar1->length) + j) * scalar1->element_size);
            complex* n1 = mat1;
            complex* n2 = newComplex(n1->realPart * 5, n1->complexPart * 5);
